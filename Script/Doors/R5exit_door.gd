...
func _ready():
	...
	if not GameProgress.room5_both_done.is_connected(open_door):
		GameProgress.room5_both_done.connect(open_door)

	if GameProgress.room5_analyst_solved and GameProgress.room5_coder_solved:
		open_door()
